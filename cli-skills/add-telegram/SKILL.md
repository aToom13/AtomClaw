...
This skill adds Telegram support to atomclaw, then walks through interactive setup.
...
```bash
git remote add telegram https://github.com/qwibitai/atomclaw-telegram.git
```
...
```bash
npm run build
launchctl kickstart -k gui/$(id -u)/com.atomclaw  # macOS
# Linux: systemctl --user restart atomclaw
```
...
- Check bot is started: `tail -f logs/atomclaw.log`
...
6. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.atomclaw` (macOS) or `npm run build && systemctl --user restart atomclaw` (Linux)
