---
name: setup
description: Run initial atomclaw setup. Use when user wants to install dependencies, authenticate messaging channels, register their main channel, or start the background services. Triggers on "setup", "install", "configure atomclaw", or first-time setup requests.
---
...
**Case A — `origin` points to `qwibitai/atomclaw` (user cloned directly):**

The user cloned instead of forking. AskUserQuestion: "You cloned atomclaw directly. We recommend forking so you can push your customizations. Would you like to set up a fork?"
- Fork now (recommended) — walk them through it
- Continue without fork — they'll only have local changes

If fork: instruct the user to fork `qwibitai/atomclaw` on GitHub (they need to do this in their browser), then ask them for their GitHub username. Run:
```bash
git remote rename origin upstream
git remote add origin https://github.com/<their-username>/atomclaw.git
...
```bash
git remote add upstream https://github.com/qwibitai/atomclaw.git
```
...
**Verify:** `git remote -v` should show `origin` → user's repo, `upstream` → `qwibitai/atomclaw.git`.
...
2. **Fresh start** — "Skip migration and set up atomclaw from scratch."
...
- macOS: `launchctl unload ~/Library/LaunchAgents/com.atomclaw.plist`
- Linux: `systemctl --user stop atomclaw` (or `systemctl stop atomclaw` if root)

Run `npx tsx setup/index.ts --step service` and parse the status block.

**If FALLBACK=wsl_no_systemd:** WSL without systemd detected. Tell user they can either enable systemd in WSL (`echo -e "[boot]\nsystemd=true" | sudo tee /etc/wsl.conf` then restart WSL) or use the generated `start-atomclaw.sh` wrapper.
...
- macOS: check `launchctl list | grep atomclaw`. If PID=`-` and status non-zero, read `logs/atomclaw.error.log`.
- Linux: check `systemctl --user status atomclaw`.
...
- SERVICE=stopped → `npm run build`, then restart: `launchctl kickstart -k gui/$(id -u)/com.atomclaw` (macOS) or `systemctl --user restart atomclaw` (Linux) or `bash start-atomclaw.sh` (WSL nohup)
...
Tell user to test: send a message in their registered chat. Show: `tail -f logs/atomclaw.log`

## Troubleshooting

**Service not starting:** Check `logs/atomclaw.error.log`. Common: wrong Node path (re-run step 7), credential system not running (Docker: check `curl ${ONECLI_URL}/api/health`; Apple Container: check `.env` credentials), missing channel credentials (re-invoke channel skill).

**Container agent fails ("Claude Code process exited with code 1"):** Ensure the container runtime is running — `open -a Docker` (macOS Docker), `container system start` (Apple Container), or `sudo systemctl start docker` (Linux). Check container logs in `groups/main/logs/container-*.log`.

**No response to messages:** Check trigger pattern. Main channel doesn't need prefix. Check DB: `npx tsx setup/index.ts --step verify`. Check `logs/atomclaw.log`.

**Channel not connecting:** Verify the channel's credentials are set in `.env`. Channels auto-enable when their credentials are present. For WhatsApp: check `store/auth/creds.json` exists. For token-based channels: check token values in `.env`. Restart the service after any `.env` change.

**Unload service:** macOS: `launchctl unload ~/Library/LaunchAgents/com.atomclaw.plist` | Linux: `systemctl --user stop atomclaw`
